...
namespace mtca4u {
   ...
   */
  template<class T>
  typename ProcessScalarImpl<T>::SharedPtr createSimpleProcessScalar(
   ...
   */
  template<class T>
  typename std::pair<typename ProcessScalarImpl<T>::SharedPtr,
      typename ProcessScalarImpl<T>::SharedPtr> createSynchronizedProcessScalar(
      const std::string & name = "", T initialValue = 0,
  ...
  typename std::pair<typename ProcessScalarImpl<T>::SharedPtr,
    ...
    boost::shared_ptr<typename impl::ProcessScalarImpl<T> > receiver =
        boost::make_shared<typename impl::ProcessScalarImpl<T> >(
            ...
            numberOfBuffers);
    typename ProcessScalarImpl<T>::SharedPtr sender = boost::make_shared<
        typename impl::ProcessScalarImpl<T> >(
        impl::ProcessScalarImpl<T>::SENDER, timeStampSource,
        sendNotificationListener, receiver);
    return std::pair<typename ProcessScalarImpl<T>::SharedPtr,
        typename ProcessScalarImpl<T>::SharedPtr>(sender, receiver);
  }
